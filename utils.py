...
def decrypt_data(encrypted_data, key):
    try:
        ...
        logger.info(f"Nonce: {b64encode(nonce).decode('utf-8')}")
        logger.info(f"Tag: {b64encode(tag).decode('utf-8')}")
        logger.info(f"Ciphertext: {b64encode(ciphertext).decode('utf-8')}")
